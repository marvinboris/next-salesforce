...
const ManageRolesPage: NextPageWithLayout = () => {
    ...
    const data = (backend && backend[resource] ? (backend[resource] as RoleInterface[]) : []).map(role => updateObject(role, {
        created_at: convertDate(role.createdAt!),
        action: <Action props={props} resource={resource} item={role} />,
    }));
...
ManageRolesPage.getLayout = (page: ReactElement) => <Layout>{page}</Layout>
