import { HomeIcon, UserGroupIcon, PaperAirplaneIcon, UsersIcon, ArchiveBoxIcon } from '@heroicons/react/24/outline'
        ...
        <main className='flex-1'>
            ...
            <div className="px-[33px] xl:px-[42px] pt-[29px] xl:pt-[47px] pb-[54px]">
                ...
                <div className="grid gap-2.5 md:grid-cols-2 xl:grid-cols-4 mb-[51px] lg:mb-[35.08px]">
                    ...
                    <StatCard color='blue' title={blocks.transactions}>
                        <div className="flex items-end justify-between">
                            <div className='pb-[7px] md:pb-[5px]'>
                                <span className="font-bold text-3xl md:text-lg">{data && data.blocks && data.blocks.transactions}</span>
                            </div>

                            <div><PaperAirplaneIcon className='w-20 md:w-11 text-blue/20' /></div>
                        </div>
                    </StatCard>

                    <StatCard color='sky' title={blocks.clients}>
                        <div className="flex items-end justify-between">
                            <div className='pb-[7px] md:pb-[5px]'>
                                <span className="font-bold text-3xl md:text-lg">{data && data.blocks && data.blocks.clients}</span>
                            </div>

                            <div><UsersIcon className='w-20 md:w-11 text-sky/20' /></div>
                        </div>
                    </StatCard>

                    <StatCard color='yellow' title={blocks.products}>
                        <div className="flex items-end justify-between">
                            <div className='pb-[7px] md:pb-[5px]'>
                                <span className="font-bold text-3xl md:text-lg">{data && data.blocks && data.blocks.products}</span>
                            </div>

                            <div><ArchiveBoxIcon className='w-20 md:w-11 text-black/10' /></div>
                        </div>
                    </StatCard>
                </div>
            </div>
...
ManagerDashboardPage.getLayout = (page: ReactElement) => <Layout>{page}</Layout>
