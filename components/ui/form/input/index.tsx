...
export default function Input({ icon: Icon, label, addon, append, inputSize = 'lg', className, ...props }: InputProps) {
    return <div className={className}>
        ...
        <div className={classNames(inputSize === 'sm' ? "h-12" : "h-12 md:h-[60px]", "rounded-[300px] relative bg-secondary-100 flex items-center")}>
            <div>
            ...
            <div className={classNames(inputSize === 'sm' ? Icon || addon ? 'pr-5' : 'px-5' : Icon || addon ? 'pr-8' : 'px-8', 'flex-1 relative z-0')}>
                <input {...props} className={classNames(inputSize === 'sm' ? 'text-sm' : 'text-sm md:text-lg', 'min-h-[48px] border-none bg-transparent outline-none text-inherit w-full placeholder:opacity-30')} />
            </div>
