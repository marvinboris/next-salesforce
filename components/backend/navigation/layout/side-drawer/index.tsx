...
import { AdjustmentsHorizontalIcon, ComputerDesktopIcon, UserPlusIcon, Cog8ToothIcon } from "@heroicons/react/24/outline";
                        ...
                        <div>
                            <div className="space-y-3">
                                ...
                                {data && 'role' in data && data.role.features.filter(({ prefix }) => resources.includes(prefix)).map(({ prefix }) => {
                                    ...
                                    return <NavItem key={JSON.stringify(cms.sidebar.menu[resource as ResourceType])} icon={resourceIcon(resource as ResourceType)} href={`/${role}/${prefix}`}>{cms.sidebar.menu[resource as ResourceType].title}</NavItem>
                                })}
